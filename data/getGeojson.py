...
initDf_safe = pd.read_csv(
    r"D:\000_VU_Study\P4\IV\project\d3_js_learn\liu\ams_safety_index_districts.csv")
...
safeDf2.rename(columns={
               'polygon_of_district_2_in_area': 'polygon_of_district_1_in_area'}, inplace=True)
...
safeDf3.rename(columns={
               'polygon_of_district_3_in_area': 'polygon_of_district_1_in_area'}, inplace=True)
...
for year in years:
    initDf_stats['score_people_'+year] = initDf_stats.apply(lambda row: computeScore(
        row['people_working_'+year], row['population_'+year]), axis=1)
    ...
    initDf_stats['score_people_norm_' +
                 year] = initDf_stats['score_people_norm_'+year]*100
    # normallization move
    ...
    initDf_stats['score_move_norm_' +
                 year] = initDf_stats['score_move_norm_'+year]*100

for year in years:
    initDf_stats['population_stability_score_'+year] = initDf_stats['score_move_norm_'+year] + initDf_stats['score_people_norm_'+year]
